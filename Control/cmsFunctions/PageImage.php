...
class PageImage
{
    ...
    public function page_image($page_id=PAGE_ID, $options=array())
    {
        ...
        elseif (defined('POST_ID') && (POST_ID > 0)) {
            ...
            $SQL = "SELECT `content_long` FROM `".CMS_TABLE_PREFIX."mod_news_posts` WHERE `post_id`=".POST_ID;
        ...
        elseif (defined('TOPIC_ID') && (TOPIC_ID > 0)) {
            // this is a TOPICS Article
            $SQL = "SELECT `content_long` FROM `".CMS_TABLE_PREFIX."mod_topics` WHERE `topic_id`=".TOPIC_ID;
            $content = $this->app['db']->fetchColumn($SQL);
            $content = $this->app['tools']->unsanitizeText($content);
            if (false !== ($image = $this->getImage($content))) {
            ...
            $SQL = "SELECT `picture` FROM `".CMS_TABLE_PREFIX."mod_topics` WHERE `topic_id`=".TOPIC_ID;
